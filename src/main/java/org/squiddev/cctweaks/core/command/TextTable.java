...
import static org.squiddev.cctweaks.core.command.ChatHelpers.coloured;
import static org.squiddev.cctweaks.core.command.ChatHelpers.text;
...
public class TextTable {
	...
	public static int getWidth(IChatComponent text, ICommandSender sender) {
		...
		int actual = Minecraft.getMinecraft().fontRenderer.getStringWidth(text.getUnformattedText());
		if (actual != sum) {
	...
	private final List<IChatComponent[]> rows = Lists.newArrayList();

	public TextTable(@Nonnull IChatComponent... header) {
		this.header = header;
	...
	public TextTable(@Nonnull String... header) {
		this.header = new IChatComponent[header.length];
		for (int i = 0; i < header.length; i++) {
			this.header[i] = ChatHelpers.header(header[i]);
		}
	}

	public void addRow(@Nonnull IChatComponent... row) {
		if (columns == -1) {
			columns = row.length;
	...
	public void displayTo(ICommandSender sender) {
		...
		if (header != null) {
			...
			out.appendSibling(coloured(StringUtils.repeat('=', rowWidth), EnumChatFormatting.GRAY));
			out.appendSibling(LINE);
		}

		for (int i = 0; i < rows.size(); i++) {
			IChatComponent[] row = rows.get(i);
	...
	private static void appendFixed(IChatComponent out, ICommandSender sender, IChatComponent entry, int maxWidth) {
		...
		if (delta < 0) {
			...
			IChatComponent component = new ChatComponentText(StringUtils.repeat(' ', spaces < 0 ? 0 : spaces));
			if (missing > 0) {
				IChatComponent bold = new ChatComponentText(StringUtils.repeat(' ', missing));
				bold.getChatStyle().setBold(true);
